          ...
          {blogPosts.map((post, i) => {
            ...
            return (
              <Reveal key={post.slug} delay={(i % 2) * 0.1}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="group flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-white/10 bg-black transition-all duration-500 hover:border-[#4C0C0A]/60 hover:shadow-2xl transform hover:-translate-y-1"
                >
                  <div>
                    <div className="relative aspect-[16/9] w-full overflow-hidden bg-stone-900">
                      ...
                      <div className="absolute bottom-4 left-4 right-4 flex items-center gap-2 text-white">
                        <Calendar className="h-4 w-4 text-[#4C0C0A]" />
                    ...
                    <div className="p-8">
                      <h2 className="font-display text-xl font-bold uppercase tracking-wide text-white group-hover:text-[#4C0C0A] transition-colors">
                        {post.title}
                      </h2>
                  ...
                  <div className="mx-8 mb-8 pt-4 border-t border-white/10 flex items-center justify-between font-body text-xs font-bold uppercase tracking-widest text-[#4C0C0A] group-hover:translate-x-1 transition-transform">
