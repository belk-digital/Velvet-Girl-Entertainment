        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start">
          ...
          <div className="flex flex-col py-4">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm font-semibold mb-8">
              <div className="flex items-center gap-1.5 text-[#4C0C0A]">
              ...
              <span className="text-stone-300">|</span>
              <span className="text-[#4C0C0A] uppercase">{eventsCount}</span>
            </div>

            {/* Attributes */}
            <div className="mb-8">
              <h3 className="text-[#4C0C0A] font-bold text-sm mb-3">Attributes:</h3>
            ...
            {performer.services && performer.services.length > 0 && (
              <div className="mb-10">
                <h3 className="text-[#4C0C0A] font-bold text-sm mb-4">Available Specialties:</h3>
                <div className="flex flex-wrap gap-6 sm:gap-10">
                  {performer.services.map((service: string) => {
                    const Icon = SERVICE_ICONS[service.toLowerCase()] ?? PartyPopper;
                    return (
                      <div key={service} className="flex flex-col items-center gap-2 text-[#4C0C0A]">
            ...
            {(performer.tagline || performer.bio) && (
              <div className="mb-10">
                <h3 className="text-[#4C0C0A] font-bold text-sm mb-3">About</h3>
                {performer.tagline && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <a href="tel:8439387377" className="flex items-center justify-center gap-2 bg-[#4C0C0A] hover:bg-[#5c0911] text-white py-4 rounded-md font-bold text-sm tracking-widest transition-colors shadow-sm">
                <Phone className="w-4 h-4" />
                CALL / TEXT
              </a>
              <Link href={`/book-now?performer=${performer.slug || performer.id}`} className="flex items-center justify-center gap-2 bg-white hover:bg-black text-[#4C0C0A] border border-white/10 py-4 rounded-md font-bold text-sm tracking-widest transition-colors shadow-sm">
