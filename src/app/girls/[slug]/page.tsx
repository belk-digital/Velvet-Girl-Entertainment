...
export default async function PerformerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] font-body text-white pb-24 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 sm:pt-32 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start">
          ...
          <div className="flex flex-col py-4">
            <h1 className="font-display text-5xl sm:text-6xl md:text-7xl font-bold tracking-tight text-white mb-3">
              {performer.name}
            </h1>
            
            <div className="flex items-center gap-2 text-white font-bold tracking-widest text-sm uppercase mb-4">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm font-semibold mb-8">
              <div className="flex items-center gap-1.5 text-[#380605]">
              ...
              <span className="text-stone-300">|</span>
              <span className="text-[#380605] uppercase">{eventsCount}</span>
            </div>

            {/* Attributes */}
            <div className="mb-8">
              <h3 className="text-[#380605] font-bold text-sm mb-3">Attributes:</h3>
              <div className="flex flex-wrap gap-2">
                ...
                {performer.hairColor && (
                  <span className="px-4 py-1.5 rounded-full border border-white/10 text-stone-300 text-sm font-medium capitalize shadow-sm">
            ...
            {performer.services && performer.services.length > 0 && (
              <div className="mb-10">
                <h3 className="text-[#380605] font-bold text-sm mb-4">Available Specialties:</h3>
                <div className="flex flex-wrap gap-6 sm:gap-10">
                  {performer.services.map((service: string) => {
                    const Icon = SERVICE_ICONS[service.toLowerCase()] ?? PartyPopper;
                    return (
                      <div key={service} className="flex flex-col items-center gap-2 text-[#380605]">
            ...
            {(performer.tagline || performer.bio) && (
              <div className="mb-10">
                <h3 className="text-[#380605] font-bold text-sm mb-3">About</h3>
                {performer.tagline && (
                  <p className="text-white font-semibold mb-2">{performer.tagline}</p>
                )}
                {performer.bio && (
                  <p className="text-stone-300 leading-relaxed whitespace-pre-line">{performer.bio}</p>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <a href="tel:8439387377" className="flex items-center justify-center gap-2 bg-[#380605] hover:bg-[#5c0911] text-white py-4 rounded-md font-bold text-sm tracking-widest transition-colors shadow-sm">
                <Phone className="w-4 h-4" />
                CALL / TEXT
              </a>
              <Link href={`/book-now?performer=${performer.slug || performer.id}`} className="flex items-center justify-center gap-2 bg-white hover:bg-black text-[#380605] border border-white/10 py-4 rounded-md font-bold text-sm tracking-widest transition-colors shadow-sm">
