...
export default function AgeGate({ onVerified }: AgeGateProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-[#4C0C0A]/10 backdrop-blur-[2px]"></div>

      <div className="relative z-10 mx-4 w-full max-w-3xl sm:max-w-4xl">
        <div
          data-gate-card
          className="rounded-[28px] border border-white/10 bg-[#4C0C0A]/95 px-5 py-6 text-center text-white opacity-0 shadow-[0_8px_40px_rgba(0,0,0,0.5)] backdrop-blur-xl sm:px-12 sm:py-10"
        >
          ...
          >
            <button
              type="button"
              onClick={handleEnter}
              className="group flex w-full items-center justify-center gap-2 rounded-full bg-white px-8 py-2.5 sm:py-3.5 font-body text-sm font-bold tracking-caps text-[#4C0C0A] transition-all duration-300 hover:scale-[1.02] hover:bg-[#111] hover:shadow-lg sm:w-auto sm:flex-1"
            >
              ENTER SITE
