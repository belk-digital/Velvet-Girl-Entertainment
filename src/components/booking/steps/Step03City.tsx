  ...
  return (
    <StepLayout stepKey={3} title="Where is the event?">
      <div className="max-w-md my-8">
        <label
          htmlFor="city-select"
          className="mb-3 block font-body text-xs font-bold uppercase tracking-widest text-stone-400"
        >
          Event City
        </label>
        <div className="relative">
          <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-[#5C0005]">
      ...
      <div className="mt-12 mb-10 border-t border-white/10 pt-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#5C0005]/10 text-[#5C0005] text-xs font-bold uppercase tracking-wider mb-2">
            ...
            <p className="font-body text-sm text-stone-300 mt-1 max-w-2xl">
          ...
          {selectedList.length > 0 && (
            <div className="flex items-center gap-2 bg-[#5C0005] text-white px-4 py-2 rounded-full font-bold text-xs sm:text-sm shadow-md">
        ...
        {/* Performers Grid */}
        {!state.city ? (
          <div className="rounded-2xl border border-dashed border-white/10 bg-black px-6 py-10 text-center">
            <p className="font-body text-sm text-stone-300">
              Select an event city above to see local entertainers.
            </p>
          </div>
        ) : filteredPerformers.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[#5C0005]/30 bg-[#5C0005]/5 px-6 py-10 text-center">
            <p className="font-body text-sm font-semibold text-white">
              We don&apos;t have a local roster listed for{" "}
              {state.city.split(",")[0]} yet.
            </p>
            <p className="font-body text-sm text-stone-300 mt-1 max-w-md mx-auto">
            ...
            return (
              <div
                ...
                className={`group relative flex flex-col justify-between bg-black rounded-[22px] border transition-all duration-300 overflow-hidden shadow-md hover:shadow-2xl hover:-translate-y-1.5 ${
                  isSelected
                    ? "border-[#5C0005] ring-2 ring-[#5C0005]/30 bg-red-50/20"
                    : "border-white/10/90"
                }`}
              >
                <Link
                  ...
                  target="_blank"
                  className="relative aspect-[3/4] w-full overflow-hidden bg-[#111] cursor-pointer block"
                >
                  <Image
                    src={performer.image}
                    alt={performer.name}
                  ...
                  {isSelected && (
                    <div className="absolute top-3.5 right-3.5 z-10">
                      <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#5C0005] text-white shadow-lg">
                  ...
                  <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white/90 backdrop-blur-md text-xs font-bold text-white shadow-lg">
                      <Eye className="w-3.5 h-3.5 text-[#5C0005]" />
                      View Photos & Bio
                    </span>
                  </div>
                ...
                <div className="p-4 flex flex-col justify-between flex-grow gap-4">
                  <div className="flex items-center justify-between text-xs text-stone-300">
                    ...
                    {performer.hairColor && (
                      <span className="px-2 py-0.5 rounded-md bg-[#111] font-medium text-[11px]">
                        {performer.hairColor}
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 px-4 rounded-xl font-body text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center justify-center gap-2 shadow-sm ${
                      isSelected
                        ? "bg-[#5C0005] text-white shadow-md hover:bg-[#8B0000]"
                        : "bg-stone-900 text-white hover:bg-[#5C0005]"
                    }`}
                  >
                    {isSelected ? (
                      <>
