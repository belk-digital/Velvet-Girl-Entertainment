...
class Gallery {
  constructor(container) {
    ...
    // Pointer events — scoped to the gallery's own container, not the whole window,
    // so hovering elsewhere on the page doesn't drive the parallax.
  ...
  getViewportWidth() {
    return this.container?.clientWidth || (typeof window !== "undefined" ? window.innerWidth : 1)
  }
  ...
  getPlaneLabelData(planeDefinition, index) {
    const fallback = {
      word: `performer ${String(index + 1).padStart(2, '0')}`,
    }
  ...
  updatePlaneScale() {
    const isMobileViewport = this.getViewportWidth() <= this.mobileBreakpoint
  ...
  updatePlaneMotion(scroll = null) {
    ...
    this.planes.forEach((plane, index) => {
      ...
      const aspectRatio = plane.userData.aspectRatio || 1
      const baseScale = isMobileViewport ? this.mobilePlaneScale : this.desktopPlaneScale
