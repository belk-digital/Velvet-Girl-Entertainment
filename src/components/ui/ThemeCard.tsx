...
export default function ThemeCard({ theme }: { theme: PackageTheme }) {
  const content = (
    <>
      <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden bg-[#380605]">
        {theme.image ? (
          <>
            <Image
      ...
      <div className="p-6 flex flex-col flex-1 justify-between bg-black border-t border-stone-100">
        <div>
          <h3 className="font-display text-xl font-bold text-white group-hover:text-[#380605] transition-colors uppercase tracking-wide">
            {theme.name}
          </h3>
        ...
        {!theme.comingSoon && (
          <div className="mt-6 pt-4 border-t border-white/10 flex items-center justify-between font-body text-xs font-semibold uppercase tracking-widest text-[#380605] group-hover:translate-x-1 transition-transform">
  ...
  return (
    <Link
      ...
      className="group flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-black transition-all duration-500 hover:border-[#380605]/60 hover:shadow-2xl transform hover:-translate-y-1"
    >
      {content}
    </Link>
  );
